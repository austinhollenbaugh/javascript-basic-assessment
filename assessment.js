...
//returning the array with one item left in it, even if it matches the string


//#11 Write a function doubleTheFun that takes 1 parameter. it doubles numbers, and
// repeats strings. Example 4->8, 2.5->5, 'Awesome'->'AwesomeAwesome'

function doubleTheFun(thingToDouble) {
  if (typeof thingToDouble === 'number') {
    thingToDouble = thingToDouble * 2;
      return thingToDouble;
  } else if (typeof thingToDouble === 'string') {
    if (isNaN(thingToDouble)) {
      return thingToDouble + thingToDouble;
    } else {
...
function makeChatMessage(messageP, authorP) {
  var obj = {
    message: messageP,
    author: authorP,
    timestamp: new Date(),
